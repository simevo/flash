...
base_url = "https://www.avvenire.it/"
feed_id = 113
...
for a in soup.select(".Articolo a"):
    url = a.get("href")
    if url[0] == "/":
        url = base_url + url
    entries.append(url)
for a in soup.select(".Editoriale a"):
    url = a.get("href")
    if url[0] == "/":
        url = base_url + url
    entries.append(url)
for a in soup.select(".Rubrica a"):
    url = a.get("href")
    if url[0] == "/":
        url = base_url + url
    entries.append(url)
for a in soup.select(".Intervista a"):
    url = a.get("href")
    if url[0] == "/":
        url = base_url + url
    entries.append(url)
...
for e in entries:
    ...
    soup = BeautifulSoup(html, "lxml")
    stamp = soup.select("meta[itemprop=datePublished]")[0].get("content")
    title = soup.select("div.page_content > h1")[0].get_text()
    author = soup.select("span.author")[0].get_text().strip()
    content = soup.select("div[itemprop=articleBody]")[0].prettify()
